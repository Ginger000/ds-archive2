...
class KDTree
{
  ...
  public:
    ...
    int quickSelectHelper(vector<Point<Dim>>& points, int left, int right, unsigned pivotIndex, int dimension);
    KDTreeNode* constructorHelper(vector<Point<Dim>>& points, int dimension, unsigned left, unsigned right);
   Point<Dim> quickSelect(vector<Point<Dim>>& points, int left, int right, int n, int dim);
    /**
    ...
    KDTree<Dim>::KDTreeNode* copy(KDTreeNode* subRoot);
    /**
     ...
     */
    ~KDTree();
    void deleteHelper(KDTreeNode* subRoot);
    ...
    void findHelper(KDTreeNode* curr, Point<Dim> & best, const Point<Dim>& query,int dim )const;
