...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    // if (key == subroot->elements[first_larger_idx]) {
    //     return subroot->elements[first_larger_idx].value;
    // }

    if(subroot->elements.empty()!=true&&!(first_larger_idx>=subroot->elements.size()))
   	{
...
void BTree<K, V>::insert(const K& key, const V& value)
{
     ...
     * root node. */
    //why compare the size of the rootnode instead of the exact node where the new pair is inserted
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    if (subroot->is_leaf)
    {
        ...
        //where to put this split function???
