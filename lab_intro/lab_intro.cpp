...
for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      if (distance <= 160.0)
      {
        double discreasedValue = 0.005 * distance;
        // double remainValue = 1 - discreasedValue;
        //!!! The reason I get wrong is one varible only could be declared once.
        remainValue = 1 - discreasedValue;
        // pixel.l = pixel.l * remainValue;
      } else {
